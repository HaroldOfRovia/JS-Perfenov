...
button.addEventListener("click", function() {
  ...
  var epsilon = document.getElementById("epsilon").value;//CHANGE
  ...
  table(Number(hh), Number(n), A, B, C, D, perX, perY, alfa, betta, gamma, delta, epsilon);//CHANGE

  if(f.checked){
    printf(A, B, C, D, perX, perY, alfa, betta, gamma, delta, epsilon);//CHANGE
  }
  ...
  if(r.checked){
    printr(A, B, C, D, perX, perY, alfa, betta, gamma, delta, epsilon, hh, n);
  }

  if(df.checked){
    printdf(A, B, C, D, perX, perY, alfa, betta, gamma, delta, epsilon, hh, n, del);
  }
...
//строит обычную функцию
function printf(A, B, C, D, perX, perY, alfa, betta, gamma, delta, epsilon){//CHANGE FIN

  ctx.fillStyle = "#006400"; 

  //  Number(alfa)*Math.sin(Math.tan(Number(betta)*x))+Number(gamma)*Math.cos(Number(delta)*x);
  for(var x = Number(A); x <= Number(B); x = x+0.01/perX){
    var y = Number(alfa)*Math.sin(Math.tan(Number(betta)/(x-Number(gamma)))+Number(delta)*Math.cos(Number(epsilon)*x));//CHANGE
...
function pol(t, k, n) {
    ...
    if (k < n)
        return (pol(t, k + 1, n) + (fac(t, k) * matr[n-k][k]));
...
function table(hh, n, A, B, C, D, perX, perY, alfa, betta, gamma, delta, epsilon) {//CHANGE
//первый столбец (func(x))
  for (var x = Number(A), i = 0; i <= n; x += hh, i++) {
    //Number(alfa)*Math.sin(Math.tan(Number(betta)*x))+Number(gamma)*Math.cos(Number(delta)*x);
      matr[i][0] = Number(alfa)*Math.sin(Math.tan(Number(betta)/(x-Number(gamma)))+Number(delta)*Math.cos(Number(epsilon)*x));//CHANGE
  }
...
//отрисовка Rn()
function printr(A, B, C, D, perX, perY, alfa, betta, gamma, delta, epsilon, hh, n) {
...
function printdf(A, B, C, D, perX, perY, alfa, betta, gamma, delta, epsilon,  hh, n, del) {
    ...
    for (var x = Number(A); x <= Number(B); x += 0.01) {
        ...
        var s = Number(alfa)*Math.sin(Math.tan(Number(betta)/((x+Number(del))-Number(gamma)))+Number(delta)*Math.cos(Number(epsilon)*(x+Number(del))));//CHANGE 
